...
object LogSugar {
  ...
  internal fun log(tag: String?, details: Any?, breadcrumb: Throwable? = null) {
    logInternal(tag ?: getFunctionBreadcrumb(), details.toString(), breadcrumb)
  }

  internal fun logMultiple(
    tag: String? = null,
    details: Collection<Any?>,
    breadcrumb: Throwable? = null,
  ) {
    val seq = details.asSequence()
    rawLogInternal(tag ?: getFunctionBreadcrumb(), breadcrumb) { chunkSize ->
      seq.flatMap {
        it.toString().windowedSequence(size = chunkSize, step = chunkSize, partialWindows = true)
      }
    }
  }
...
fun log(details: Any? = "") = LogSugar.log(null, details)

/** Print a debug message (pass null for [tag] to get autogenerated breadcrumb) */
fun log(tag: String?, details: Any?, breadcrumb: Throwable? = null) =
  LogSugar.log(tag, details, breadcrumb)

/** Print a multi-line debug message */
fun logMultiple(tag: String?, details: Collection<Any?>) = LogSugar.logMultiple(tag, details)

/** Print a multi-line debug message */
fun logMultiple(details: Collection<Any?>) = logMultiple(null, details)
 ...
 */
fun <T> T.inspect(tag: String? = null): T {
  return when (this) {
    is Collection<*> -> inspectEach(tag = tag)
    is Map<*, *> -> inspectEach(tag = tag)
    is Flow<*> -> inspectEach(tag = tag) as T
    is Observable<*> -> inspectEach(tag = tag) as T
    is Single<*> -> inspectEach(tag = tag) as T
    is Completable -> inspectEach(tag = tag) as T
    else -> inspect(tag) { it.toString() }
  }
}
 ...
 * someObject.getThing().inspect("thing").title
 */
fun <T> T.inspect(tag: String? = null, toString: ((T) -> Any?)): T {
